...
//       // Add your logic to fetch mutual friends for the given userId
//       const mutualFriends = await MutualFriendsModel.find({ userId }); // Example logic
//       res.status(200).json({ data: mutualFriends });
//     } catch (error) {
...
const checkUserAuth = async(req, res) =>{
     try {
        ...
        return response(res,201, 'user retrived and allow to use Unseen Perspectives', user)
     } catch (error) {
        return response(res, 500, 'Internal server error', error.message)
     }
}
